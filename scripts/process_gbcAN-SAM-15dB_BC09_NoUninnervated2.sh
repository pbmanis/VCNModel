...
CELLCONFIG="NoUninnervated2"  # or "Full"
...
do
    ...
    do
        ...
        if [[ ($freq -lt "500") ]]  # account for runs all completed for lower frqeuencies
        then
        ...
        python vcnmodel/model_run2.py VCN_c$cell -D $CELLCONFIG $AXON -P initAN $CONFIG $TABLE $TEST
        python vcnmodel/model_run2.py VCN_c$cell -D $CELLCONFIG $AXON -P runANPSTH --fmod $freq $DMOD $REPS $EXP1 $CONFIG $TABLE $TEST $WKN
        if [ $? -ne 0 ]; then
            exit 1
        fi
