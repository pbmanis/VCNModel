...
FILES="VCN_c02 VCN_c05 VCN_c11 VCN_c13"
for f in $FILES
do
    echo "Cell: $f"

    python src/model_run.py VCN_c17 -i $f --protocol runANSingles -H \
         -r 1 -d 40 -f 4000. --sgcmodel cochlea -S MS --noparallel --configfile autoscale.toml\
             --saveall
done
wait
