...
sprint(topofile, "calyx-S53Acvt2.hoc") 
system("pwd", ocmd) // get the working directory - we move around... 
sscanf(ocmd, "%s\n", o2cmd) // strip trailing \mn
sprint(lfcmd, "%s/%s\n", o2cmd, topofile)

//load_file(1, "calyx-S53Acvt2.hoc") // load the morphology file
load_file(1, "calyx-S53Acvt3.hoc") // load the morphology file


clamp_flag = 0 // 0 = current clamp, 1 = voltage clamp
strdef clampstr
sprint(clampstr, "CC")
...
newCm = 1 // uf/cm2 cap.
newRa = 100 // changed 10/20/2007 to center in range
newg_leak = 0.000004935 
axonnode = 149 // the heminode section (this may vary with model type)

// define which sections (and where in the section) to monitor
// currents, voltages, and ion concentrations
// this should be changed on a per-calyx configuration, but right no2
// we nave only one (S53) to work with.
objref monsecs, monpos
monsecs = new Vector(0)
monpos = new Vector(0)
// sections to monitor
	monsecs.insrt(0, 0,\
	149, \
	...
	130, \
	117)
monpos.insrt(0, 0,	0, 1.0, 0.5, 0.5, 0.5, 0.5)
...
objref secid
objref swelldist
dia_thresh = 0.25 // minimum diameter of any part... adjustable in menu
vsaveflag = 0
isaveflag = 0
mark_sw = 1
mark_br = 1
...
nswel = 200
nactual_swel = 0
objref vswel[nswel], caswel[nswel], icaswel[nswel], vclampi, tdat, vax[3] 
...
load_file(1, "calyx_g_control.hoc")
load_file(1, "calyx_experiments.hoc")
load_file(1, "calyx_electrodes.hoc")
load_file(1, "calyx_morpho.hoc")

load_file(1, "calyx_biophysics.hoc")
load_file(1, "mesh.hoc")
load_file(1, "calyx_shape.hoc")

proc reload() {

	load_file(1, "calyx_experiments.hoc")

	load_file(1, "calyx_tune.hoc")
}
...
proc calyx_init() { local dtsav, temp

// First we set e_leak so that the rmp in each segment is the same
	finitialize(v_init)
	forsec all { // set a new value
		e_newleak = v+(ina + ik + ica + i_ih)/g_leak
...
// based on those conditions, make sure spatial grid is fine enough for our needs
	mesh_init()
	finitialize(v_init) // recompute
	if(GBCFLAG == 1) {
		soma { icgbc.loc(0.5) }
	}
	{
		axon[axonnode] { ic.loc(iclocation) }
	}
	
	forsec parentaxon { nseg = 11 }
	forsec heminode { nseg = 11 }
//	forsec all {nseg = 1 }
	axon[axonnode] {	
		vc.loc(vclocation)
	}
...
// save the calcium pump information
	forsec all {
		...
		savtau = tau_capmp
		tau_capmp = 1e-6 // make the pump go really fast
	}

// starting way back in time
	t = -1e10
	dtsav = dt
	dt=1e9 // big time steps for slow process
	temp = cvode.active()
	if(temp!= 0) {cvode.active(0)} // turn cvode off (note, in this model it will be off because one of the mechanisms is not compatible with cvode at this time
	while(t <-1e9) {  
		fadvance()
	}
...
proc calyxrun() { local i, j, k, b
	rundone = 0
	biophys(0) // force update of all gmax values 

	dt = 0.010 // force small time step. cvode is probably off.
...
// set up the graphs
	...
	gc.begin()
	gi.begin()
	npts = 1 + int(tstop/dt) // number of points in a run
	for j = 0, monsecs.size-1 {
		sprint(scmd, "axon[%d].v(%.1f)", monsecs.x[j], monpos.x[j])
...
celldef() // includes biophysics

// if(GBCFLAG == 1) { defineGBC() } // include the GBC?
biophys(1) // make sure we are all set.

measure() // get distances
RestoreDefaultConductances() // canonical conductances... 
setdefaultconductances() // initialize the conductances
insert_clamps() // electrodes

// load anciallary functions that may need all the definitions above
load_file(1, "calyx_tune.hoc") // requires clamps to be inserted..
calyx_init() // this also sets nseg in the axon - so do it before setting up shapes
shape_graphs() // must happen AFTER calyx_init
