...
#
def doubleexp(x, amp=1.0, C0=5, C1=1, tau0=20.0, tau1=0.2):
    """
...
class Electrotonic:
    ...
    def fitexps(self, x, y, amp=5):
        ...
        y : np.array : voltage array

        Returns
        -------
        Nothing

        Stores fit result in class variable `fitresult`
        """
    ...
    def coeffs_ratio(self):
        """
        comptue the coeffients ration from the fitresult after
        fitexps
        ...
        Returns
        --------
        Coefficients ratio: (C1*tau0/(2.0*C0*tau1))
        """
    ...
    def print_pars(self):
        ...
        print(
            "tau0 (Rn*Cm): {0:.3f}".format(
                self.Rn * 1e6 * self.somaAreaMeasured * self.Cm * 1e-8
            )
        )
...
#            print("i, taud/taui: ", i, self.tau_d/self.fitresult.params["tau%d" % i].value)
 ...
 #       print('alphas taud: ', a, self.tau_d)
    ...
    def alphas_Ctau(self):
        """
        Comptues the alphas from Equations 1 and 2 from Rose and Dagum 1988
        """
        a = np.zeros(2)
        for i in range(0, 2):
            a[i] = (
                self.fitresult.params["C%d" % i].value
    ...
    def eta(self, alpha=None):
        """
        Computes eta from alpha or class variable self.alphas
        """
        if alpha is None:
            alpha = self.alphas()
        a = alpha
    ...
    def ASt(self):
        """
        Computes eq. 12, White et al. 1994
        """
    ...
    def Ct(self, i):
        """
        Calulate theroetical Ct for a give set of values
        note: coth = 1/tanh
        """
        a = self.alphas()
...
class CellPars:
    Cell: str = ""
    Rn: float = 0.0  # input resistance
    Vm: float = 0.0  # resting membrane potential
    Ih: float = 0.0  # holding current
    C0: float = 0.0  # amplitude of slow tau
    tau0: float = 0.0  # time constant of slow tau
    C0: float = 0.0  # amplitude of slow tau
    tau0: float = 0.0  # time constant of slow tau
    CR: float = 0.0  # coefficients ration.
...
def WYM94data(data):
    """
    Read a the text string of data, and create a
    data structure of CellPars
    """
...
def generate_testdata(noise=0.0):
    """
    Generate some test data - a 2-exponential decay with noise
    """
    rate = 0.01
    dur = 50.0
    npts = int(dur / rate)
    ...
    V = np.zeros(npts) - 60.0

    C_Ratios = np.array([5.0, 1.0])
...
def test():
    ...
    npx = 20
    lr = np.arange(npx) * (3.0 / npx) + 0.01
    tr = np.arange(npx) * (100.0 / npx) + E.fitresult.params["tau%d" % 0].value
    ...
    iax = mpl.imshow(
        ...
        origin="lower",
        cmap=cm.gray,
        extent=(np.min(lr), np.max(lr), np.min(tr), np.max(tr)),
        aspect="auto",
    )
    ax = iax.axes
