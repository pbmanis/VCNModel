...
1. Title of Dataset:
    Computational models/results of serial-blockface electron microscopic
...
2014-01-01 to 2022-08-30

4. Geographic location of data collection:

    Chapel Hill, NC USA, Morgantown, WV, Tampa, FL.
...
class BuildDataSet():
    ...
    def do_copy(self, src, dest, verbose=False):
        self.bytecount += src.stat().st_size  # sum up file sizes for transfer
        if self.testmode:
            cpc = 'm'
            ...
            # check that the dest date is more recent than dest
            # otherwise do nothing (just report it in yellow)
            ...
            if not dest.is_file() or (src.stat().st_size != dest.stat().st_size) or (src.stat().st_mtime > dest.stat().st_mtime):
                ...
                cpc = "y"   
                cprint(cpc, f"        File exists and date, size match:")
                cprint(cpc, f"           from: {str(src.parent):s}")
                cprint(cpc, f"               + {str(src.name):s}" )           
    ...
    def copy_morphology(self, allcells, sdirs, cdirs):
        for cn in allcells:
            ...
            for f in source_morph:
                ...
                # else:
                #     cprint('r', f"Morphology file {cell_id:s}  {str(f):s}  already present")
                #     cprint('r', f"    dest file was: {str(dest_file):s}")
                ...
                for cell_idx in list(figd.keys()):
                    ...
                    for iexpt, expt_label in enumerate(expts):  # for each expt in the list
                        ...
                        if not sourcefilepkl.is_file():
                            cprint("r", "Failed on : {str(sourcefilepkl):s}")
                            exit()
                        ...
                        if sourcefile.is_dir() and simcopy:  # make and fill the subdirectory
                            ...
                            for f in sourcefiles:
                                filelist.append({'src':f, "dest": targetsubdir}) 
                                # self.do_copy(f, targetsubdir)
                        if sourcefilepkl.is_file() and simcopy:
                            filelist.append({'src': sourcefilepkl, "dest": targetdir})
                            # self.do_copy(sourcefilepkl, targetdir})
                    for file in filelist:
...
def main():
    ...
    cprint("g", f"Bytes to write: {int(MD.bytecount):,d}")
