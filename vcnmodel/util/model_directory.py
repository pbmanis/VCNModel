...
class ModelDirectory(object):
        ...
        for d in vcndirs:
            ...
            if sim_d.is_dir:
                ...
                for f in iv_sims:
                    ...
                    datestr = datetime.datetime.fromtimestamp(mtime).strftime('%Y.%m.%d-%H:%M:%S')
                    # print(mtime)
    ...
    def backup_morphology(self):
        """
        Reads through the morphology directories under VCN_Cells, and creates
        a date-named subdirectory that has a copy of the current
        .hoc, .hocx and .swc files
        Files retain a creation and modification date that are
        set to the modification dates in the parent directory (creation
        dates cannot be easily kept, unless we use rsync)
