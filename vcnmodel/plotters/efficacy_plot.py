...
0	9	0	1017	1113	0.91375	245.76	2.52	196
1	9	1	949	1083	0.87627	132.27	2.52	106
2	9	2	935	1142	0.81874	107.92	2.52	86
3	9	3	823	1145	0.71878	74.43	2.52	59
4	9	4	790	1100	0.71818	71.50	2.52	57
5	9	5	716	1080	0.66296	62.98	2.52	50
6	9	6	652	1100	0.59273	54.81	2.52	44
7	9	7	249	1090	0.22844	35.03	2.52	28
...
8	10	8	0	1144	0.00000	42.08	2.92	34
9	10	9	0	1125	0.00000	40.09	2.92	32
0	11	0	1004	1104	0.90942	213.09	2.44	170
1	11	1	1013	1112	0.91097	156.38	2.44	125
2	11	2	987	1120	0.88125	134.46	2.44	107
3	11	3	877	1094	0.80165	89.76	2.44	72
4	11	4	824	1108	0.74368	77.04	2.44	62
5	11	5	278	1119	0.24844	36.76	2.44	29
6	11	6	293	1096	0.26734	35.67	2.44	29
0	13	0	869	1104	0.78714	146.57	2.50	117
1	13	1	562	1112	0.50540	94.11	2.50	75
2	13	2	557	1120	0.49732	91.38	2.50	73
3	13	3	547	1094	0.50000	90.88	2.50	73
4	13	4	337	1108	0.30415	77.02	2.50	62
5	13	5	58	1119	0.05183	57.42	2.50	46
6	13	6	64	1096	0.05839	56.98	2.50	46
...
0	30	0	1013	1146	0.88394	172.14	2.65	138
1	30	1	802	1074	0.74674	88.27	2.65	71
2	30	2	781	1159	0.67386	79.11	2.65	63
3	30	3	780	1123	0.69457	78.50	2.65	63
4	30	4	656	1060	0.61887	66.28	2.65	53
5	30	5	625	1075	0.58140	63.15	2.65	50
6	30	6	548	1099	0.49864	59.21	2.65	47
7	30	7	222	1080	0.20556	42.12	2.65	34
8	30	8	193	1144	0.16871	41.92	2.65	33
...
class EfficacyPlots(object):
    def __init__(self, parent_plot:object=None, hold=False, cols=3):
        self.parent_plot = parent_plot
        self.hold = hold
        self.plot_efficacy(cols)

    def plot_efficacy(self, cols):
        ...
        for i, data in enumerate(datasets):
            ...
            ax=self.P.axdict[self.axes_labels[axn[i]]], title=self.titles[axn[i]])
        return self.P
    ...
    def plot_dataset(self, data, plotno: int = 0, ax: object = None, title: str = None):
        spc = re.compile("[ ;,\t\f\v]+")  # format replacing all spaces with tabs
        dataiter = re.finditer(spc, data)
        data = re.sub(spc, ",", data)
        ...
        gmodel.set_param_hint("vhalf", value=100.0, min=10.0, max=300.0)
        gmodel.set_param_hint("k", value=5, min=0.01, max=200.0, vary=True)
        ...
        for meth in resdict.keys():
            result_LM = gmodel.fit(
                x,
                method=meth,
                params=result_brute.params,  # gparams,
                x=y,
                weights=weights,
        ...
        ax.text(
            200.0,
            result_LM.params["A"].value - 0.05,
            f"LM: Max Eff: {result_LM.params['A'].value:.2f} (1$\sigma$ = {result_LM.params['A'].stderr:.2f})",
            fontsize=8,
