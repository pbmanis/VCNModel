...
class Calyx8():
    def __init__(self, argsin=None):
        ...
        h.load_file(os.getcwd() + "/NeuronHocFiles/custom_init.hoc")  # replace init with one that gets closer to steady state
        ...
        self.runInfo = Params(fileName='Normal', runName='CalyxTest', manipulation="Canonical", folder="Simulations",
        ...
        self.modelPars = Params(
            structureNames=['parentaxon', 'axon', 'heminode', 'synapse', 'stalk', 'branch', 'neck', 'swelling', 'tip'],
            # names of calyx morphological parts
            structureColors={'axon': 'red', 'heminode': 'green', 'stalk': 'yellow', 'branch': 'green', 'neck': 'blue',
            ...
            AN_delay={'stellate': 0.0, 'bushy': 0.0, 'test': 0.0, 'DStellate': 0.3, 'MNTB': 0.3},
            # netcon delay (fixed)
            AN_lat_Flag={'stellate': 0, 'bushy': 1, 'test': 0, 'DStellate': 0, 'MNTB': 1},
            # 1 to allow latency shift during train
            AN_lat_A0={'stellate': 0.14, 'bushy': 0.140, 'test': 0.25, 'DStellate': 0.15, 'MNTB': 0.14},
            # magnitude of latency change, if allowed
            AN_lat_tau={'stellate': 21.0, 'bushy': 21.5, 'test': 21.0, 'DStellate': 21.0, 'MNTB': 21.5},
            # rate at which latency changes, if allowed
            AN_latency={'stellate': 0.5, 'bushy': 0.5, 'test': 0.5, 'DStellate': 0.5, 'MNTB': 0.5},  # starting latency
            AN_relstd_Flag={'stellate': 1, 'bushy': 2, 'test': 0, 'DStellate': 0, 'MNTB': 1},
            # flag controlling std shift during train (1 to enable)
            AN_relstd={'stellate': 0.05, 'bushy': 0.05, 'test': 0.05, 'DStellate': 0.05, 'MNTB': 0.05},
            # baseline release std
            AN_relstd_A0={'stellate': 0.0, 'bushy': 0.05, 'test': 0.0, 'DStellate': 0.0, 'MNTB': 0.05},
            # magnitude of std change during train, if allowed
            AN_relstd_tau={'stellate': 35.0, 'bushy': 35.0, 'test': 35.0, 'DStellate': 35.0, 'MNTB': 35.0},
            # time constant of std change during train, if allowed
            ...
            INH_lat_Flag={'stellate': 0, 'bushy': 0, 'test': 0, 'DStellate': 0},
            # 1 to allow latency shift during train
            INH_latency={'stellate': 0.2, 'bushy': 0.2, 'test': 0.2, 'DStellate': 0.2},
            # latency for stochastic synapses
            INH_relstd_Flag={'stellate': 1, 'bushy': 2, 'test': 0, 'DStellate': 0},
            # flag controlling std shift during train (1 to enable)
        ...
        self.modelPars.axonnode = self.modelPars.monsecs[
            1]  # the heminode section (this must be specified in this way in the topofile dict)

        defPars = self.restoreDefaultConductances()  # set the default conductance for each section from scratch
        self.modelPars.gPars = self.setDefaultConductances(
            defPars=defPars)  # assign the current "defaults" to the relevant variables...
        if verbose:
            print 'modelPars initialization done'
        self.rundone = 0
        self.thisrep = 1
        ...
        self.nswel = 200
        self.nactual_swel = 0
        success = h.load_file(1, 'MorphologyFiles/' + self.modelPars.topofile)  # load the morphology file
        if success == 0:
            print 'Could not find morphology file: %s' % (self.modelPars.topofile)
            return
        if verbose:
        ...
        for input in range(self.modelPars.AN_conv[self.runInfo.TargetCellName]):
            for swellno, swell in enumerate(self.CalyxStruct[input]['swelling']):
                ...
                swelling = self.getAxonSec('swelling', swellno, input)
                (calyx, coh, psd, cleft, nc2, par) = Synapses.stochastic_synapses(h,
                                                                                  parentSection=
                    self.CalyxStruct[input]['axon'][swelling],
                                                                                  targetcell=self.TargetCell,
                                                                                  cellname=self.runInfo.TargetCellName,
                                                                                  nFibers=
                                                                                  self.modelPars.AN_conv[
                                                                                      self.runInfo.TargetCellName],
                                                                                  nRZones=
                                                                                  self.modelPars.AN_zones[
                                                                                      self.runInfo.TargetCellName],
                                                                                  message='creating stochastic multisite synapse',
                                                                                  thresh=self.modelPars.Exc_Thresh,
                                                                                  psdtype='ampa',
                                                                                  gmax=
                    self.modelPars.AN_gEPSC[self.runInfo.TargetCellName] * 1000.,
                                                                                  gvar=
                                                                                  self.modelPars.AN_gvar[
                                                                                      self.runInfo.TargetCellName],
                                                                                  eRev=
                                                                                  self.modelPars.AN_erev[
                                                                                      self.runInfo.TargetCellName],
                                                                                  NMDARatio=self.modelPars.NMDARatio,
                                                                                  debug=False, Identifier=1,
                                                                                  stochasticPars=self.modelPars.stochasticPars,
                                                                                  calciumPars=None)
        ...
        if verbose:
            print 'built synapses'

        if renderFlag:
            pg.mkQApp()
            pg.dbg()
            render = nv.HocViewer(h)
            render.hr.read_hoc_section_lists(self.modelPars.structureColors.keys())
            surface = render.draw_surface()
            surface.set_group_colors(self.modelPars.structureColors, mechanism=self.modelPars.mechNames['na'])
            #render.show()
    ...
    def setDefaultConductances(self, defPars=None, gPars={}):
        ...
        for name in self.modelPars.structureNames:
            gPars[name] = Params(gNabar=defPars.gna_def, ENa=defPars.eNa_def, gKLbar=defPars.glvk_def,
                                 EK=defPars.eK_def,
                                 gKHbar=defPars.ghvkax_def, gCabar=defPars.gca_def, gHbar=defPars.gh_def,
                                 gCaPump=defPars.capump_def).todict()
            # exceptions:
    ...
    def biophys(self, runInfo=None, modelPars=None, structure=None, createFlag=True):
        """
    ...
    def calyx_init(self, runInfo=None, modelPars=None):
        ...
        for input in range(self.modelPars.AN_conv[runInfo.TargetCellName]):
            for sec in self.CalyxStruct[input]['axon']:  # set a new value
                if i == 0:
                    i += 1
                e_newleak = sec.v + (sec.ina + sec.ik + +sec.ik_klt + sec.ik_kht + sec.ica + sec.i_ih) / (
    ...
    def getMonSec(self, monsec):
        ...
        for s in self.modelPars.structureNames:
            ...
            for element in self.CalyxStruct[0][s]:
                ...
                axno = g.groups()[0]
                if (int(axno)) == monsec:
                    self.clist[monsec] = self.modelPars.structureColors[s]
                    #                   print 'match found for %d in %s' % (monsec, s)

    def getAxonSec(self, structure, number, input, type='axon'):
        """
        Get the axon (type) section in the calyx structure at position
        Inputs:
            structure: valid name of dictionary element of calyx tags
            number: the index into the structure
        Returns: the axon(type) number (if found).
        Side effects: None.
        """
        src = re.compile('%s\[(\d*)\]' % type)
    ...
    def calyxrun(self, runInfo=None, modelPars=None):
        ...
        print 'calyxrun'
        self.vec = {}
        for var in ['axon', 'stalk', 'branch', 'neck', 'tip', 'swelling',]: # get morphological components
            self.vec[var] = h.Vector()
        for var in ['inj', 'time', 'postsynaptic', 'i_stim0']: # get standard stuff
            self.vec[var] = h.Vector()
        ...
        for i in range(nconv):
            ...
            if self.runInfo.inFile is None:
                ...
                stim['PT'] = 0.0
                (secmd, maxt, tstims) = makestim(stim, pulsetype='square', dt=h.dt)
                istim[0].delay = 0
                ...
                (secmd, maxt, tstims) = makestim(stim, pulsetype='timedSpikes', dt=h.dt)
                istim[i].delay = 0
        ...
        cplts = cp.CalyxPlots()
        cplts.plotResults(results.todict(), runInfo.todict())
        cplts.show()
    ...
    def runModel(self, runInfo=None, modelPars=None):
        ...
        self.calyx_init(runInfo=runInfo,
                        modelPars=modelPars)  # this also sets nseg in the axon - so do it before setting up shapes
