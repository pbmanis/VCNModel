...
class hocRender():
    def __init__(self, h, fileName=None):
        self.h = h
        self.app = pg.mkQApp()
        ...
        if fileName is None:
            ...
            self.w.setWindowTitle(fileName)
    ...
    def makeVolumeData(self, x, y, z, d, lines, threshold=0.0):
        res = 0.2 # resolution of scalar field in microns
        ...
        for c in range(len(lines)-3):
            ...
            for k in range(nvoxels):
                ...
                stamp_array(scfield, np.clip(kernel+(dia/2.0)+threshold, 0, np.inf), p1)
                dia += (d[j]-d[i]) / nvoxels
                p1 += diff / nvoxels
    ...
    def drawBlob(self, x, y, z, d, lines):
        scfield, transform = self.makeVolumeData(x, y, z, d, lines, threshold=1.0)
        scfield = scipy.ndimage.gaussian_filter(scfield, (0.5, 0.5, 0.5))
        pg.image(scfield)
        verts, faces = pg.isosurface(scfield, level=1.0)
    ...
    def drawMeshes(self, x, y, z, d, lines, mode):
        ...
        for c in range(len(lines)-3):
            ...
            if mode == "line":
                ...
                md = gl.MeshData.cylinder(rows=2, cols=12, radius=[d[i]/2., d[j]/2.], length=cyllen)
                colors = np.ones((md.faceCount(), 4), dtype=float)*np.array([0.2, 0.2, 0.6, 0.08])
                #colors[:,3] = 0.3 # alpha
 #               colors[::2,0] = 0
 #               colors[:,1] = np.linspace(0, 1, colors.shape[0])
...
if __name__ == "__main__":
    pg.mkQApp()
    #pg.dbg()
#    fileName = "Calyx-S53Acvt3.hoc"
#    fileName = 'Calyx-68cvt2.hoc'
#    fileName = "LC_nmscaled_cleaned.hoc"
    fileName = "L23pyr.hoc"
    h.load_file(1, fileName)
    render = hocRender(h, fileName)
    render.draw_model(modes=['cylinder', 'line'])
    render.show()
