...
import src.cell_config as cell_config
import neuronvis.hoc_graphics as hoc_graphics
from src.generate_run import GenerateRun
...
#            print('getting initial conditions for AN')
            ...
            #                     self.initDirectory, self.Params['initANStateFile'])
            cellInit.get_initial_condition_state(post_cell, tdur=self.Params['initialization_time'],
        ...
        if self.Params['Parallel']:
            with MP.Parallelize(enumerate(TASKS), results=tresults,
...
#        total_run_time = time.time() - run_time
    ...
    def single_an_run(self, post_cell, j, synapseConfig, stimInfo, seeds, preCell, an_setup_time):
        ...
        try:
            ...
            print('single_an_run: could not restore initial conditions: will try to create again')
            self.an_run(post_cell, make_an_intial_conditions=True)
    ...
    def plot_an(self, celltime, result):
        ...
        for j, N in enumerate(range(len(result))):
            ...
            ax[0].plot([np.min(celltime[N]), np.max(celltime[N])], [threshold, threshold],
        ...
        if 'dendVoltage' in result[N].keys() and result[N]['dendVoltage'] is not None:
            for j, N in enumerate(range(len(result))):
                ax[0].plot(celltime[N], result[N]['dendVoltage'][0], c='b', linewidth=0.6, linestyle='--')
        mpl.show()
